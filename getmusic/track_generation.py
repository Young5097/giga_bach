...
from getmusic.utils.misc import (
    seed_everything,
    merge_opts_to_config,
    modify_config_for_debug,
)
...
from getmusic.utils.magenta_chord_recognition import (
...
os.chdir("C:/github/giga_bach/giga_bach/getmusic")
...
# python track_generation.py --load_path checkpoint.pth --file_path example_data/inference
root_dict = {
    "C": 0,
    "C#": 1,
    "D": 2,
...
def normalize_to_c_major(e):
    ...
    e = [
        tuple(k + pitch_shift if j == 3 and i[2] != 128 else k for j, k in enumerate(i))
        for i in e
    ]
...
def MIDI_to_encoding(midi_obj, with_chord, condition_inst, chord_from_single):
    ...
    for i in range(len(tsc)):
        for j in range(
            time_to_pos(tsc[i].time),
            time_to_pos(tsc[i + 1].time) if i < len(tsc) - 1 else max_pos,
        ):
            if j < len(pos_to_info):
                pos_to_info[j][1] = t2e(
                    time_signature_reduce(tsc[i].numerator, tsc[i].denominator)
                )
    for i in range(len(tpc)):
        for j in range(
            time_to_pos(tpc[i].time),
            time_to_pos(tpc[i + 1].time) if i < len(tpc) - 1 else max_pos,
        ):
            if j < len(pos_to_info):
...
def encoding_to_MIDI(encoding, tpc, decode_chord):
    ...
    for i in range(len(bar_to_timesig)):
        if bar_to_timesig[i] is None:
            bar_to_timesig[i] = (
                t2e(time_signature_reduce(4, 4)) if i == 0 else bar_to_timesig[i - 1]
            )
...
def F(file_name, conditional_tracks, content_tracks, condition_inst, chord_from_single):
    ...
    # try:
    encoding, pitch_shift, tpc = MIDI_to_encoding(
        midi_obj, with_chord, condition_inst, chord_from_single
    )
    ...
    while idx != len(encoding) - 1:
        ...
        if e[2] == 129:
            ...
            k = kind_list[e[4]]
            datum[2 * row][
                pos_in_bar * bar + pos : pos_in_bar * (bar + 1) + pos
            ] = tokens_to_ids[r]
            datum[2 * row + 1][
                pos_in_bar * bar + pos : pos_in_bar * (bar + 1) + pos
            ] = tokens_to_ids[k]
    ...
    datum = torch.where(
        ((datum != empty_index).float() * (1 - conditional_bool)).type(torch.bool),
        empty_index + 1,
        datum,
    )
    ...
    for i in range(14):
        if (
            with_chord
            and conditional_tracks[i] == 1
            and ((datum[i] == pad_index).sum() + (datum[i] == empty_index).sum())
            == min(512, figure_size)
        ):
            have_cond = False
            break
...
def main():
    ...
    file_list = [
        os.path.join(args.file_path, n)
        for n in os.listdir(args.file_path)
        if (n[-4:].lower() == ".mid" or n[-5:].lower() == ".midi")
    ]  # and ('iter' not in n.lower())]
    file_list.sort()
    ...
    for file_name in file_list:
        ...
        conditional_name = input(
            "Select condition tracks ('b' for bass, 'd' for drums, 'g' for guitar, 'l' for lead, 'p' for piano, 's' for strings, 'c' for chords; multiple choices; input any other key to skip):"
        )
        condition_inst = []
        ...
        if all(conditional_track):
            print(
                "You can't set all tracks as condition. We conduct uncontional generation based on selected content tracks. If you skip content tracks, this song is skipped."
            )

        content_track = np.array([False, False, False, False, False, False, False])
        content_name = input(
            "Select content tracks ('b' for bass, 'd' for drums, 'g' for guitar, 'l' for lead, 'p' for piano, 's' for strings; multiple choices):"
        )
        ...
        if all(conditional_track):
            print(
                "You can't set all tracks as condition. We conduct uncontional generation based on selected content tracks."
            )
            conditional_track = np.array(
        ...
        x, tempo, not_empty_pos, condition_pos, pitch_shift, tpc, have_cond = F(
            file_name,
            conditional_track,
            content_track,
            condition_inst,
            args.chord_from_single,
        )
        ...
        oct_line = solver.infer_sample(
            x, tempo, not_empty_pos, condition_pos, use_ema=args.no_ema
        )
